...
###1. Стандартная Main функция выглядит следующим образом.
```csharp
using System;
...
```
Замените строку "ВАШ_ТЕЛЕГРАМ_БОТ_ТОКЕН" на ваш токен, полученный через BotFather.

###2. Напишем сразу HandleErrorAsync для обработки ошибок бота и чтобы он не ломался.
```csharp
...
```
###3. Теперь приступим к написанию основоного функционала (HandleUpdateAsync) который может обрабатывать команды, получать медиафайлы от пользователей и отвечать на выбор callback-кнопок.
   Основной метод программы — `HandleUpdateAsync` — обрабатывает все обновления от Telegram API, такие как сообщения и callback-запросы.
   ###3.1. Определение `chatId` для дальнейшей работы с ботом
```csharp
long chatId;
...
###3.2 Обработка callback-запросов.
```csharp
...
```
В этом блоке кода обрабатываются callback-запросы от пользователя. Callback-запросы создаются при нажатии на кнопки Inline-клавиатуры, которая выводится позже в коде. В зависимости от нажатой кнопки, бот отправляет пользователю стикер, GIF, фото или видео-кружок.
###3.3. Ожидание команды для загрузки файла.
```csharp
...
```
Здесь бот ожидает команду /add от пользователя. Как только команда получена, бот отправляет сообщение с запросом на загрузку файла и переводит переменную isWaitFile в true, чтобы бот понимал, что ожидается загрузка медиафайла.
Этой функцией мы загружаем на сервер телеграмма наш файл в дальнейшем используя file_id мы отправляем жти файлы пользователям.
###3.4  Обработка загруженных файлов.
```csharp
if (isWaitFile)
{
...
```
Этот блок кода обрабатывает файлы, загруженные пользователем. В зависимости от типа загруженного файла (видео-кружок, фото, анимация или стикер), бот получает file_id и отправляет его обратно пользователю. После получения файла ожидание файла прекращается (isWaitFile = false).
###3.5. Создание Inline-клавиатуры.
```csharp
...
```
В этом блоке создается Inline-клавиатура, содержащая четыре кнопки. Каждая кнопка отправляет callback-запрос с определенным идентификатором (например, sticker_1 для первого стикера), который обрабатывается в предыдущих блоках.
